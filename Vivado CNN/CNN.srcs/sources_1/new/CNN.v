...
module CNN( output reg [9:0]A, input wire clk);
    ...
    always@(posedge clk)	// Initializing Convolution on the image
        if(flag_conv) begin
            if(it == 0) begin
                ...
                K1 = Filters[0];
                K2 = Filters[1];
                K3 = Filters[2];	//	First row
                K4 = Filters[3];
                K5 = Filters[4];
                K6 = Filters[5]; 	//	Second row
                K7 = Filters[6];
                K8 = Filters[7];
                K9 = Filters[8]; 	//	Third row
                ...
                if(Conv[it] >= max_value)
                    Conv[it] = max_value;			//}
                else if( Conv[it] <th )
                    Conv[it] = alpha*(Conv[it]-th);	//} Appling ReLu

                it = it+1;
                i = i+1;
                j = j+1;
                k = k+1;	// Updation of iterators
            end
            else if( it == 900 ) begin
                flag_conv = 0;
                flag_max = 1;
                it = 0;
            end
    ...
    always@(posedge clk)	// Initializing Calculation of final features only if flag_final == 1
        if(flag_final) begin
            if(t == 10) begin
                ...
                sum = 0;
                j = 0;
                t=t+1;		// 	resetting the iterators
            end
            else begin
                if(j)
                    sum = sum + (MP[j]*Weights[225*t+j]);
                else
                    sum = MP[0]*Weights[225*t];
    ...
    always@(posedge clk)	// FPGA Basys-3 Board output generation initialization only if flag_prob == 1
        if(flag_prob) begin
            max = 0;
            if(Final[1]>Final[max])
                max = 1;	// }
            if(Final[2]>Final[max])
                max = 2;	// }
            if(Final[3]>Final[max])
                max = 3;	// }
            if(Final[4]>Final[max])
                max = 4;	// }
            if(Final[5]>Final[max])
                max = 5;	// }	finding the maxima among feautures
            if(Final[6]>Final[max])
                max = 6;	// }
            if(Final[7]>Final[max])
                max = 7;	// }
            if(Final[8]>Final[max])
                max = 8;	// }
            if(Final[9]>Final[max])
                max = 9;	// }

            A = 0;
            A[max] = 1;		// Setting the detected bit, turning off other bits
